...
tests :: TestTree
tests = testGroup "URI" [validURITests, invalidURITests]

validURITests :: TestTree
validURITests = testGroup "valid" [completeURITests, prefixURITests]

completeURITests :: TestTree
completeURITests = testProperty "parse_entire_string" $ do
  uriText <- Text.pack . show <$> genURI
  pure $ fmap serialize (parse @URI uriText) === Just uriText

prefixURITests :: TestTree
prefixURITests = testProperty "parse_prefix" $ do
  uriText <- Text.pack . show <$> genURI
...
strayPrefixTests = testProperty "stray_prefix" $ do
  uriText <- Text.pack . show <$> genURI
...
strayInfixCharTests = testProperty "stray_infix" $ do
  uriText <- Text.pack . show <$> genURI
...
straySuffixCharTests = testProperty "stray_suffix" $ do
  uriText <- Text.pack . show <$> genURI
  suffix <- elements nonURITexts
